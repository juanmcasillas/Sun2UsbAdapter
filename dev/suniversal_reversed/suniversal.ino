...
void updateLEDs() {
  ...
  uint8_t ledsSUN = ((ledsUSB & USB_LED_CAPS_LOCK) << 2) | (ledsUSB & (USB_LED_NUM_LOCK | USB_LED_SCROLL_LOCK));
  ...
  if (ledsUSB != ledsUSBPrev) {
    DPRINTLN("suniversal: USB LED state changed: " + String(ledsUSBPrev, HEX) + " --> " + String(ledsUSB, HEX));
    ledsUSBPrev = ledsUSB;
  }

  if (cmdLED[1] != ledsSUN) {
    DPRINTLN("suniversal: SUN LED state changed: " + String(cmdLED[1], HEX) + " --> " + String(ledsSUN, HEX));
    cmdLED[1] = ledsSUN;
    sun.write(cmdLED, 2);
  }
}
